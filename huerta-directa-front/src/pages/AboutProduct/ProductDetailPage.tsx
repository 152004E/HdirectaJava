...
export const ProductDetailPage = () => {
  ...
  const fetchComments = async () => {
    try {
      const response = await fetch(`http://localhost:8085/api/comments/product/${id}`);
      if (response.ok) {
    ...
    const fetchProduct = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.category) {
            const relatedResponse = await fetch(`http://localhost:8085/api/products/category/${encodeURIComponent(data.category)}`);
            if (relatedResponse.ok) {
              const relatedData = await relatedResponse.json();
              const filtered = relatedData.filter((p: Product) => p.idProduct !== Number(id));
  ...
  if (!product) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen py-20 gap-6">
    ...
    <div className="bg-gray-50 min-h-screen pb-20 pt-10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-0">
            ...
            <div className="lg:col-span-8 p-6 lg:p-10 border-r border-gray-100">
              <div className="flex flex-col md:flex-row gap-6">
                ...
                {/* main Image */}
                <div className="flex-1 bg-gray-50 rounded-2xl overflow-hidden flex items-center justify-center relative min-h-[400px]">
                  <img 
                    src={mainImageUrl} 
              ...
              <div className="mt-12 hidden lg:block">
                <div className="flex border-b border-gray-100 mb-6">
                  ...
                  >
                    Descripción
                    {activeTab === "description" && <div className="absolute bottom-0 left-0 w-full h-1 bg-[#8dc84b] rounded-t-full"></div>}
                  </button>
                  <button 
                  ...
                  >
                    Reseñas ({reviewCount})
                    {activeTab === "reviews" && <div className="absolute bottom-0 left-0 w-full h-1 bg-[#8dc84b] rounded-t-full"></div>}
                  </button>
                </div>
                ...
                <div className="min-h-[200px]">
                  {activeTab === "description" ? (
                    <div className="prose max-w-none text-gray-600 leading-relaxed">
                      <p className="whitespace-pre-line">{product.descriptionProduct}</p>
                    </div>
                  ) : (
                            ...
                            <div key={comment.idComment} className="border-b border-gray-50 pb-6">
                              <div className="flex justify-between items-start mb-2">
                                <div className="flex items-center gap-3">
                                  ...
                                  <span className="font-bold text-gray-800 text-sm">{comment.nameCommenter || "Usuario Anónimo"}</span>
                                </div>
                                <span className="text-xs text-gray-400">{new Date(comment.creationComment).toLocaleDateString()}</span>
                              </div>
                              <div className="flex text-yellow-400 text-[10px] mb-2">
                                {[...Array(5)].map((_, i) => (
                                  <FontAwesomeIcon key={i} icon={faStar} className={i < comment.rating ? '' : 'text-gray-200'} />
                                ))}
                              </div>
                              <p className="text-sm text-gray-600 leading-relaxed italic">"{comment.commentCommenter}"</p>
                            </div>
                          ))}
                        </div>
                      ) : (
            ...
            <div className="lg:col-span-4 p-6 lg:p-8 bg-gray-50/30">
              <div className="flex flex-col h-full">
                ...
                <div className="mb-8">
                  <span className="text-4xl font-black text-[#004d00]">${product.price.toLocaleString()}</span>
                  <span className="text-gray-400 text-lg ml-2">/ {product.unit || 'unidad'}</span>
                </div>
                ...
                <div className="mt-auto space-y-4">
                  ...
                  <div className="flex items-center gap-2 justify-center text-xs text-gray-400 py-4">
                    ...
                    <p><span className="text-blue-500 font-bold">Compra Protegida</span>. Recibe el producto que esperabas o te devolvemos tu dinero.</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="border-t border-gray-100 pt-10">
                ...
                <div className="bg-gray-50 p-6 rounded-2xl border border-gray-100 mb-8">
                  ...
                  <form onSubmit={handleSubmitReview} className="space-y-4">
                    ...
                    />
                    <button className="w-full py-3 bg-[#8dc84b] text-white rounded-lg font-bold">Publicar reseña</button>
                  </form>
                </div>
                ...
                {comments.length > 0 ? (
                  <div className="space-y-6">
                    {comments.map((comment) => (
                        <div key={comment.idComment} className="pb-6 border-b border-gray-50 last:border-0">
                          <div className="flex justify-between items-center mb-2">
                            <span className="font-bold text-gray-800 text-sm">{comment.nameCommenter || "Usuario"}</span>
                          ...
                          <p className="text-sm text-gray-600 italic">"{comment.commentCommenter}"</p>
                        </div>
                    ))}
                  </div>
                ) : (
                  <p className="text-center text-gray-400 italic text-sm">No hay reseñas aún.</p>
                )}
              </div>
            </div>
          </div>
        ...
        {relatedProducts.length > 0 && (
          <div className="mt-16">
            <h2 className="text-2xl font-black text-gray-800 mb-8">Productos similares que podrían gustarte</h2>
