...
export const Footer = () => {
  return (
    <footer className="w-full bg-[#8bc34a] text-white relative">

      {/* Línea decorativa superior */}
      <div className="absolute top-0 left-0 w-full h-1 bg-linear-to-r from-green-700 via-lime-500 to-green-700"></div>
            ...
            {[faFacebookF, faInstagram, faWhatsapp, faXTwitter].map(
              (icon, index) => (
                <a
                  key={index}
                  href="#"
                  className="w-10 h-10 flex items-center justify-center rounded-full bg-white/20 hover:bg-white hover:text-[#8bc34a] transition-all duration-500 ease-in-out hover:scale-110"
                >
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm">
            <li>
              <Link
                to="/"
                className="hover:text-[#455e28] transition-all duration-500 ease-in-out"
              >
                Inicio
              </Link>
            </li>
      ...
      {/* Sub-footer */}
      <div className="bg-[#7cb342] text-center py-5 text-sm text-green-100 border-t border-white/20">
